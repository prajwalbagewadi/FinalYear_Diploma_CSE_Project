...
public class HeristicOptOnDemandResrvBaseMdl_1 {
		...
		try {
           ...
           int i;
            // st.executeUpdate("delete from result");
                    //i = st.executeUpdate("insert into result values('"+cost+"','"+perDay+"','"+threeMonths+"','"+sixMonths+"','"+oneYr+"','"+threeYr+"')");
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static List<Cloudlet> createCloudletList(int userId, int nb_cloudlet, int IdShift) { 
		...
		for (int i = no_cur_cloudlet ; i < (offset+nb_cloudlet) ; i++) 
		{
				...
				list.add(cloudlet);
				no_cur_cloudlet++;
				System.out.println("Cloudlet created // Cloudlet # =  "+ no_cur_cloudlet +"  //  Cloudlet List Size = " + list.size());
		}
		return list;
	}
